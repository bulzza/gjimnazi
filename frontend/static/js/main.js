...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showCards(startIndex) {
    var totalItems = portfolioContainer.children.length;
    ...
    for (var i = 0; i < cardsPerPage; i++) {
      if (startIndex + i < totalItems) {
        portfolioContainer.children[startIndex + i].style.display = 'block';
      }
    }
    ...
    pageButtons[0].classList.toggle('disabled', currentIndex === 0);
    pageButtons[pageButtons.length - 1].classList.toggle('disabled', currentIndex + cardsPerPage >= totalItems);
  }
  ...
  for (var i = 0; i < pageButtons.length; i++) {
    pageButtons[i].addEventListener('click', function (event) {
      ...
      if (!this.classList.contains('disabled')) {
        ...
        var startIndex = pageIndex * cardsPerPage;
